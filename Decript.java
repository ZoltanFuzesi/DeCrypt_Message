...
public class Decript {
	...
	public static void main(String[] args) {
		...
		JFrame frame = new JFrame("Decryption Message ");
		...
		JButton encr = new JButton("Decrypt");
		encr.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {

				String strM = Mess.getText();
				char[] ch = strM.toCharArray();
				int[]numb = new int[ch.length];
				...
				char[] decodedMessage = new char[ch.length];
				for(int i=0;i<numb.length;i++)
				{
					int d = numb[i];
					decodedMessage[i] = decodeMessage(d);
				}
				String outPut = "";;
	...
	public static char decodeMessage(int d)
	{
		char answ = ' ';
		int err = d;
		int decodeT = 0;
		...
		answ = getCharFromPosition(decod);
